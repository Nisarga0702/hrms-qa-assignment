    ...
    pool.getConnection((err, connection) => {
        ...
        const query = `
            SELECT
                ...
                s.Image,
                ps.ApproveBy,
                IFNULL(s.totalDeduction,0) AS totalDeduction,
                IFNULL(s.amount,0) AS BonusAmount,
                IFNULL(
                    ROUND(
                        SUM(
                            HOUR(SUBTIME(o.end_time, o.start_time))
                        ) * s.OTRate * ((s.BaseSalary/30)/7),
                    2),
                0) AS OTamount,
...
const approvePayment = (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        ...
        connection.query(
            `INSERT INTO paymentstatus (EmployeeID, ApproveBy) VALUES (?,?)`,
            [EmployeeID, ApproveBy],
            (err, result) => {
